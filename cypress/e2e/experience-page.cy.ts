...
for (let viewport of viewports) {
  context(
    ...
    () => {
          ...
          expPage
            ...
            .then(($children) => {
              for (let idx = 0; idx < $children.length; idx++) {
                ...
                expect($children.eq(idx)).to.have.css(
                  "justify-content",
                  "center"
                );
      ...
      describe(`Education experience rendering  at Viewport: ${viewportDisplay(
        viewport
      )}`, () => {
        beforeEach(() => {
          expPage.toggleExperienceSections(0);
      ...
      describe(`Work experience rendering at Viewport: ${viewportDisplay(
        viewport
      )}`, () => {
        before(() => {
        ...
        it("Work section layout is rendering correctly", () => {
          expPage
            .workContainer()
            .should("be.visible")
            .children()
            ...
            .then(($children) => {
              cy.wrap($children)
                .eq(0)
                .then(($section) => {
                  expect($section).to.have.prop("tagName", "SECTION");
                  expect($section).to.have.attr("id", "workIntro");
                });
              cy.wrap($children)
                .eq(1)
                .then(($nav) => {
                  expect($nav).to.have.prop("tagName", "NAV");
                  expect($nav).to.have.attr("id", "workToggle");
                });
              cy.wrap($children)
                .eq(2)
                .then(($ul) => {
                  expect($ul).to.have.prop("tagName", "UL");
                  expect($ul).to.have.attr("id", "workList");
                });
            });
        });
            ...
            .then(($items) => {
              cy.wrap($items)
                ...
                .then(($h1) => {
                  ...
                  expect($h1).to.have.text("Professional experience");
                });
              cy.wrap($items)
                .eq(1)
                .then(($div) => {
            ...
            .then(($items) => {
              ...
              cy.wrap($items)
                ...
                .then(($div) => {
                  ...
                  expect($div).to.have.css("justify-content", "space-evenly");
        ...
        it("Work filter layout is correct", () => {
          expPage
            .workToggle()
            .children()
            .should("have.length", 2)
        ...
        it("Work filter styling is correct", () => {
          expPage
            .workToggle()
            .children("button")
            .each(($button) => {
        ...
        it("Work filtering renders correctly", () => {
          const work = () => {
            return expPage.workToggle().children("button").eq(0);
          };
          const volunteer = () => {
            return expPage.workToggle().children("button").eq(1);
          };

          // Work active at default
          work().then(($button) => {
            expect($button)
              .to.have.css("color")
              .match(/^rgb\(0, 0, 0\)$/);
          ...
          // Volunteer inactive at default
          volunteer().then(($button) => {
            expect($button)
              .to.have.css("color")
          ...
          // Switch active filter and check opposite
          expPage.toggleWorkSections(1);
        ...
        it("Work filtering functions correctly", () => {
          expPage.workToggle().should("be.visible");
        });

        it("Work items layout is rendering correct", () => {
          expPage
            .workList()
            .should("be.visible")
            .children()
            .should("have.length.greaterThan", 1)
            .then(($item) => {
              cy.wrap($item).each(($item) => {
                expect($item).to.have.prop("tagName", "LI");

                cy.wrap($item)
                  .children()
                  .should("have.length", 2)
                  .then(($containers) => {
                    cy.wrap($containers).each(($container) => {
                      expect($container).to.have.prop("tagName", "DIV");
                    });
                    cy.wrap($containers)
                      .eq(0)
                      .then(($container) => {
        ...
        it("Work items styling is correct", () => {
          expPage
            .workList()
            .children()
            .then(($item) => {
              cy.wrap($item).each(($item) => {
                ...
                expect($item).to.have.css("color");
                expect($item).to.have.css("background-color");
                    ...
                    cy.wrap($containers)
                      ...
                      .then(($container) => {
                        ...
                        cy.wrap($container)
                          ...
                          .each(($item) => {
                            expect($item).to.have.css("position", "relative");
                            expect($item).to.have.css("margin-top", "20px");
                            expect($item).to.have.css("font-size");
                            expect($item).to.have.css("text-indent", "6px");
                          });
                      });
                  });
              });
