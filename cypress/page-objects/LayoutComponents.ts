...
export class LayoutComps {
    ...
    testNavLinkStyle(navItem: Cypress.Chainable<JQuery<HTMLElement>>, width: Cypress.Chainable<number>, idx: number) {
        navItem
            .find("a")
            .should("be.visible")
            ...
            .then(($link: JQuery<HTMLElement>) => {
                if (idx !== 5) {
                    ...
                    expect($link).to.have.css("color").match(/rgb\(0, 33, 26\).*/);
                    expect($link).to.have.css("background-color").match(/.*rgb\(36, 94, 36\).*/);
    ...
    testFooterItemStyle(footerLink: Cypress.Chainable<JQuery<HTMLElement>>) {
        ...
        let link = footerLink
            ...
            .should("be.visible")
            .then(($link: JQuery<HTMLElement>) => {
                expect($link).to.have.css("color").match(/.*rgb\(206, 255, 208\).*/);
            });
